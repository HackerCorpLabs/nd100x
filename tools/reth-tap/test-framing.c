...
int main(void) {
    ...
    pair_tap(tap); pair_sock(sock);
    {
        unsigned char frame[60];
        for (i = 0; i < 60; i++) frame[i] = (unsigned char)i;
    ...
    close(tap[0]); close(tap[1]); close(sock[0]); close(sock[1]);
    ...
    close(tap[0]); close(tap[1]); close(sock[0]);
    ...
    pair_tap(tap); pair_sock(sock);
    {
        ...
        check(pump_tap_to_wire(tap[0], sock[0]) == 0,
              "tap->wire: a 2048-byte frame is accepted");
        n = recv(sock[1], buf, sizeof buf, 0);
